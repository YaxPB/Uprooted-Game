...
[AddComponentMenu("Physics 2D Toolkit/Circle Force Field 2D",order:0)]
public class CircleForceField2D : ForceField2DUniversal {
	...
	private  float scaleFactorMultiplier = 0f;
	...
	void FixedUpdate () {
		...
		CalcualteFinalForce ();
		colliders = Physics2D.OverlapCircleAll(point, radius * scaleFactorMultiplier, layerFilter, minDepth, maxDepth);
	...
	private void HandleBasicCalculations () {
		scaleFactorMultiplier = Mathf.Max(Mathf.Abs(_transform.lossyScale.x), Mathf.Abs(_transform.lossyScale.y));
