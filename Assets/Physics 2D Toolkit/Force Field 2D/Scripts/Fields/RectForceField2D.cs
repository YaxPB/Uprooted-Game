...
[AddComponentMenu("Physics 2D Toolkit/Rect Force Field 2D",order:0)]
public class RectForceField2D : ForceField2DUniversal {
	...
	private Vector2 finalBoxSize = default(Vector2);
	...
	void FixedUpdate () {
		...
		CalcualteFinalForce ();
		colliders = Physics2D.OverlapBoxAll (point,finalBoxSize , angle, layerFilter, minDepth, maxDepth);
	...
	private void HandleBasicCalculations () {
		finalBoxSize = size;
		finalBoxSize.x *= _transform.lossyScale.x;
		finalBoxSize.y *= _transform.lossyScale.y;
